...
//            System.out.println("Not a spreadsheet doc; closing");
    ...
    public static void saveDoc(XSpreadsheetDocument sheetDoc, String fnm) { // XStorable store = Lo.qi(XStorable.class, doc);
        XComponent doc = Lo.qi(XComponent.class, sheetDoc);
        Lo.saveDoc(doc, fnm);
    }

    public static void saveDoc(XSpreadsheetDocument sheetDoc) { // XStorable store = Lo.qi(XStorable.class, doc);
        XComponent doc = Lo.qi(XComponent.class, sheetDoc);
        Lo.save(doc);
    }
...
//            System.out.println("Value is not a number or string: " + value);
            throw new RuntimeException("Value is not a number or string: " + value);
    }  // end of setVal()
    ...
    public static Object getVal(XCell cell) {
        ...
        if (cellType == CellContentType.EMPTY)
            return null;

        if (cellType == CellContentType.VALUE)
            return cell.getValue();

        if (cellType == CellContentType.TEXT)
            return cell.getFormula();

        if (cellType == CellContentType.FORMULA) {
            // Il faut évaluer la formule pour récupérer la valeur (String ou Double).
            // On utilise la fonction XCellRangeData#getDataArray() pour ce faire.
            // Cf. http://fivedots.coe.psu.ac.th/~ad/jlop/chaps/21.%20Extracting%20Data.pdf
            XCellRangeData cellContainer = UnoRuntime.queryInterface(XCellRangeData.class, cell);
            Object cellData = cellContainer.getDataArray()[0][0];
            if (cellData instanceof String) {
                String cellString = (String) cellData;
                if (cellString.trim().isEmpty()) {
                    return null;
                }
                return cellString;
            }
        ...
        throw new RuntimeException("Unknown cell cellType: " + cellType);
    }  // end of getVal()
